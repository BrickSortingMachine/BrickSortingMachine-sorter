# Supervisor Mode Implementation Plan

## Phase 1: Project Scaffolding and Setup (1-2 hours)
1.  **Create `sorter/supervisor` package:**
    -   Create a new directory `sorter/supervisor`.
    -   Add an `__init__.py` file to make it a package.
2.  **Add `supervisor` command to `sorter.py`:**
    -   Modify `sorter.py` to add a new subparser for the `supervisor` command.
    -   The subparser should accept a `--config` argument.
    -   Create a `run_supervisor` function that will be called when the `supervisor` command is used.
3.  **Create main `Supervisor` class:**
    -   Create `sorter/supervisor/supervisor.py`.
    -   Define the `Supervisor` class with an `__init__` method that takes the config path.

## Phase 2: Configuration and Service Handling (2-3 hours)
1.  **Implement `Service` data class:**
    -   Create `sorter/supervisor/service.py`.
    -   Define a `Service` class (e.g., using `@dataclass`) to store service properties.
2.  **Implement configuration loading:**
    -   In the `Supervisor` class, add a method to load the JSON config file.
...
1.  **Implement `ProcessManager`:**
    -   Create `sorter/supervisor/process_manager.py`.
...
1.  **Set up `curses` wrapper:**
    -   Create `sorter/supervisor/tui.py`.
...
## Phase 5: Core Supervisor Logic (3-5 hours)
1.  **Implement main loop in `Supervisor`:**
...
1.  **Implement log monitoring:**
    -   Create a `LogMonitor` class in `sorter/supervisor/log_monitor.py`.
...
2.  **Integrate with TUI:**
    -   The `Supervisor` will read from the message queue and pass the messages to the `TUI` for display.

## Phase 7: Testing and Finalization (3-4 hours)
1.  **Write unit tests:**
    -   Create `test/test_supervisor.py`.
...
2.  **Manual testing:**
    -   Run the supervisor with the sample config file.
