 ...
 * `enabled` (boolean, required): If `true`, the supervisor will launch this service.
 * `args` (object, required): A key-value map of command-line arguments.
 * `restart_attempts` (integer, optional): The number of times the supervisor will attempt to restart the service if it exits unexpectedly. Defaults to `0` (no restarts). A value of `-1` means infinite restarts.
 ...
 * **Session Management**: On startup, the supervisor shall generate a single, cryptographically secure random string to be used as a `SESSION_SECRET`. This secret must be passed as an environment variable to all child processes spawned by the supervisor.
 * **Startup Sequence**: The supervisor will launch services in the order they appear in the config file, respecting the `depends_on` and `startup_delay_seconds` parameters. A service will not be started until all its dependencies are in a `RUNNING` state.
 * **Automatic Restarts**: If a service with `restart_attempts` > 0 exits with an error, the supervisor will attempt to restart it, decrementing the remaining attempt count.
 * **Graceful Shutdown**: Upon receiving an interrupt signal (Ctrl+C), the supervisor shall gracefully terminate all child processes it has spawned before exiting itself.

## 5. Logging
 * **Log File Generation**: On each run, the supervisor will create a new log directory (e.g., `logs/run_2025-08-29_15-13-00/`). Inside this directory, a separate log file will be created for each enabled service (e.g., `Controller.log`, `Vision-Service.log`).
 * **Output Redirection**: All `stdout` and `stderr` from a service will be redirected to its corresponding log file.
 * **TUI Log Display**: The supervisor must scan the output of each service in real-time. Lines containing the case-sensitive strings "ERROR" or "WARN" will be captured and displayed within the TUI.

## 6. Terminal User Interface (TUI)
The supervisor shall present a continuously updating TUI for at-a-glance monitoring.

### 6.1. TUI Mockup
```
 Brick Sorter Supervisor - logs/run_2025-08-29_15-13-00
+------------------------+--------+----------+---------+----------+
