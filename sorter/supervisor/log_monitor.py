...
class LogMonitor:
    ...
    def start_monitoring(self, service: Service):
        ...
        if service.name in self._monitor_threads and self._monitor_threads[service.name].is_alive():
            return

        thread = threading.Thread(target=self._tail_log_file, args=(service,))
        thread.daemon = True  # Allows main program to exit even if these threads are running
    ...
    def _tail_log_file(self, service: Service):
        ...
        try:
            with open(service.log_file, 'r') as f:
                ...
                while not self.supervisor.is_shutting_down() and service.status not in ["STOPPED", "ERROR"]:
                    line = f.readline()
                    if not line:
            ...
            logging.error(f"Error while monitoring log for '{service.name}': {e}", exc_info=False)
